...
CREATE ENDPOINT ENDPOINT_SHAREPOINTPROD_AG
    STATE=STARTED
    AS TCP ( LISTENER_PORT = 4026 ) -- this must be different for each endpoint on the server, start with 4022
    FOR DATABASE_MIRRORING (ROLE = ALL);

CREATE LOGIN [NTDOMAIN\svc_sqldbengine] FROM WINDOWS;
GRANT CONNECT ON ENDPOINT::[ENDPOINT_SHAREPOINTPROD_AG] TO [NTDOMAIN\svc_sqldbengine];

-- Create Availability Group sql (run on primary)
CREATE AVAILABILITY GROUP SHAREPOINTPROD_AG 
   WITH ( AUTOMATED_BACKUP_PREFERENCE = PRIMARY )  -- backups will run on primary instance only
   FOR  
   REPLICA ON 
      'HPVSQL23\SHAREPOINTPROD' WITH 
         (
         ENDPOINT_URL = 'TCP://HPVSQL23.corp.hbf.com.au:4026', 
         AVAILABILITY_MODE = SYNCHRONOUS_COMMIT, 
         FAILOVER_MODE = AUTOMATIC
         ),
      'HPVSQL24\SHAREPOINTPROD' WITH 
         (
         ENDPOINT_URL = 'TCP://HPVSQL24.corp.hbf.com.au:4026',
         AVAILABILITY_MODE = SYNCHRONOUS_COMMIT,
         FAILOVER_MODE = AUTOMATIC
         );

-- Create Availability Group sql (run on standby)
ALTER AVAILABILITY GROUP SHAREPOINTPROD_AG JOIN;

-- Create listener (run on primary)
ALTER AVAILABILITY GROUP SHAREPOINTPROD_AG 
      ADD LISTENER 'HPVSQL16SPPR' ( 
	     WITH IP ( ('192.168.32.62','255.255.255.0')) , 
