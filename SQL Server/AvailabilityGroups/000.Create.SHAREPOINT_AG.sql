...
CREATE ENDPOINT ENDPOINT_SHAREPOINT_AG
    STATE=STARTED
    AS TCP ( LISTENER_PORT = 4022 ) -- this must be different for each endpoint on the server, start with 4022
    FOR DATABASE_MIRRORING (ROLE = ALL);

CREATE LOGIN [NTDOMAIN\svc_sqldbengine] FROM WINDOWS;
GRANT CONNECT ON ENDPOINT::[ENDPOINT_SHAREPOINT_AG] TO [NTDOMAIN\svc_sqldbengine];

-- Create Availability Group sql (run on primary)
CREATE AVAILABILITY GROUP SHAREPOINT_AG 
   WITH ( AUTOMATED_BACKUP_PREFERENCE = PRIMARY )  -- backups will run on primary instance only
   FOR  
   REPLICA ON 
      'HPVSQL23\SHAREPOINT' WITH 
         (
         ENDPOINT_URL = 'TCP://HPVSQL23.corp.hbf.com.au:4022', 
         AVAILABILITY_MODE = SYNCHRONOUS_COMMIT, 
         FAILOVER_MODE = AUTOMATIC
         ),
      'HPVSQL24\SHAREPOINT' WITH 
         (
         ENDPOINT_URL = 'TCP://HPVSQL24.corp.hbf.com.au:4022',
         AVAILABILITY_MODE = SYNCHRONOUS_COMMIT,
         FAILOVER_MODE = AUTOMATIC
         );

-- Create Availability Group sql (run on standby)
ALTER AVAILABILITY GROUP SHAREPOINT_AG JOIN;

-- Create listener (run on primary)
ALTER AVAILABILITY GROUP SHAREPOINT_AG 
      ADD LISTENER 'HPVSQL14SP' ( 
	     WITH IP ( ('192.168.32.68','255.255.255.0')) , 
