...
Function GetSQLLibDetails ($item)
{
	...
	Start-Process "$tempPath\db2ilist.exe" -RedirectStandardOutput $env:TEMP\DB2_Instances.tmp -wait
	$TempIn = Get-Content -Path $env:TEMP\DB2_Instances.tmp
...
Function DatabaseListCleanup ($SQLLibInstanceItem)
{
	...
	Start-Process "$env:DB2CLP\db2.exe" -ArgumentList "list active databases" -RedirectStandardOutput $env:TEMP\DB2_ActiveDatabases.tmp -wait
	$TempIn = Get-Content -Path $env:TEMP\DB2_ActiveDatabases.tmp
...
Function GetAvailableRestores ($RestoreFromFQDN,$NetbackupInstallDir)
{
    ...
    Start-Process "$NetbackupInstallDir\Netbackup\Bin\bplist.exe" -ArgumentList "-l -C $RestoreFromFQDN -t 18 -R /" -RedirectStandardOutput $env:TEMP\DB2_Netbackup.tmp -wait #-WindowStyle Hidden 
    $TempIn = Get-Content -Path $env:TEMP\DB2_Netbackup.tmp
    #Write-host "GetAvailableRestores 1.1"
    #Write-host $TempIn
   # Write-host "GetAvailableRestores 1.2"
    $RestoreItems = $TempIn | foreach{GetTimestampAndSize -RestoreItemsPart $_}
...
Function GeneratedScript_Cleanup([String]$GeneratedScript,[String]$TempDestDatabase)
{
	...
	$outfile = "$env:TEMP\DB2_GenScript_Cleaned.tmp"
	Remove-Item -Path $outfile -Recurse -Confirm:$false -ErrorAction SilentlyContinue

	# Get the database name
	$dbname = Select-String -Path $GeneratedScript -Pattern "database" -List
	...
	foreach($line in $file) {
		if (($line.Length -gt 0) -and ($line -notlike "--*" -and $line -notlike "UPDATE COMMAND*" -and $line -notlike "SET CLIENT*") -or `
			...
			$trim += "$line " -replace (";",";`f") `
							  -replace ("-- LOGTARGET '<directory>'","`fLOGTARGET 'F:\temp_backup\logtarget\$dbname'") `
...
Function GenerateButton_Click()
{
	...
	$global:SourceDatabaseSize = $null
	$TempDestInstance = (($objListBox3.SelectedItem.tostring()).split("::::"))[0]
	$TempDestDatabase = (($objListBox3.SelectedItem.tostring()).split("::::"))[4]
	$TempDestSQLLibPath = $global:InstanceList.SQLLibs.SQLLibCopyBin
	...
	$TempDBDetails | foreach{ $global:SourceDatabaseSize += [double]$_.DatabasePartSize}
	#Write-host "Database total size: "$global:SourceDatabaseSize
	If ($TempDestDatabase -eq '<<OTHER>>')
	{
		$TempDestDatabase = "XXXXXXXX"
		$TempGenScriptArguments = "restore db $TempSourceDatabase load 'D:\Program Files\VERITAS\NetBackup\bin\nbdb2.dll' taken at $TempSourceTimeStamp into $TempDestDatabase redirect generate script DB2_GenScript.tmp"
		#Write-host "Gen 1.0 ::::" $TempGenScriptArguments
		Start-Process "$env:DB2CLP\db2.exe" -ArgumentList $TempGenScriptArguments -WorkingDirectory $env:TEMP -wait -RedirectStandardOutput $env:TEMP\DB2_GenScript_REPORT.tmp
	}else{
	...
	$CleanedDBRestoreScript = GeneratedScript_Cleanup -GeneratedScript "$env:TEMP\DB2_GenScript.tmp" -TempDestDatabase $TempDestDatabase
	$RichTextBox1.text = Get-Content -Path $CleanedDBRestoreScript
...
Function Restore_Start($RestoreScript)
{
	...
	Remove-Item -Path "$env:TEMP\DB2_GenScript_Cleaned_Restore.tmp" -ErrorAction SilentlyContinue
	$tempin = $RichTextBox1.text
	$tempin | foreach {$_.replace("`f","`n")|Out-File -FilePath $RestoreScript -Encoding ascii -Append}
...
{
	$TempRestoreArguments = "list utilities show detail" # -z $env:TEMP\DB2_Restore_Completed.tmp "# > $env:TEMP\DB2_Restore_Completed.tmp
	...
	do {
		...
		$form.Refresh()
		$CompletedTemp = $null
		#Start-Process "$env:DB2CLP\db2.exe" -ArgumentList $TempRestoreArguments -NoNewWindow -RedirectStandardOutput $env:TEMP\DB2_Restore_Completed.tmp -Wait
		...
		Start-Job -Name ProgressBlock -ScriptBlock {
			...
			Start-Process "$env:DB2CLP\db2.exe" -ArgumentList $TempRestoreArguments -wait -NoNewWindow -RedirectStandardOutput $env:TEMP\DB2_Restore_Completed.tmp
		} -ArgumentList @($env:DB2CLP,$env:DB2INSTANCE)#,$RestoreScript)

		Start-sleep -Seconds 5
		$CompletedTemp = Get-Content -Path $env:TEMP\DB2_Restore_Completed.tmp
		...
		#Remove-Item -Path $env:TEMP\DB2_Restore_Completed.tmp
		#Start-sleep -Seconds 5
		$TimeOutCounter += 20
...
Function RestoreButton_Click()
{
	...
	$form.show()
	Start-Process -
	$RestoreScript = "$env:TEMP\DB2_GenScript_Cleaned_Restore.tmp"
	#$RichTextBox1.text | Out-File "$env:TEMP\DB2_GenScript_Cleaned_Restore.tmp" -Encoding ascii
...
###################Object Sizes and locations###############
	...
	$DefRichTextBoxWidth = 490
	$DefRichTextBoxHeight = 380
	###Default Progress Bar
	$DefProgressBarWidth = 1165
...
##########################Restore details selection
	...
	$objListBox2.Size = New-Object System.Drawing.Size($DefListWidth,$DefListBoxHeight) 
	...
	$objListBox3.Size = New-Object System.Drawing.Size($DefListWidth,$DefListBoxHeight) 
    ...
    {
		$TempInstanceArray += $InstanceItemTemp.Instance+'::::'+$InstanceItemTemp.Database
	}
    ...
    {
		$TempInstanceArray += $InstanceItemTemp.Instance+'::::'+'<<OTHER>>'
	}
